...
bool IDBMysql::setupDB(const QString& server, const QString& port, const QString& user, const QString& passwd,
                       bool noPasswd, const QString& name, const QString& connectionName) {
  // this is important!
  ...
  if (!QSqlDatabase::contains(connectionName)) {
    ...
    if (!noPasswd) {
      if (passwd.isEmpty()) {
        bool ok;
        QString p = QInputDialog::getText(CMainWindow::self().getBestWidgetForParent(), tr("Password..."),
                                          tr("Password for database '%1':").arg(name), QLineEdit::Password, "", &ok);
        if (!ok || p.isEmpty()) {
          return false;
        }
        db.setPassword(p);
...
bool IDBMysql::initDB() {
  ...
  QUERY_RUN(
      "CREATE TRIGGER items_insert_last_user "
      "BEFORE INSERT ON items "
      "FOR EACH ROW SET NEW.last_user = USER();",
      return false);

  QUERY_RUN(
      "CREATE TRIGGER items_update_last_user "
...
bool IDBMysql::migrateDB5to6() {
  ...
  quint32 N = query.value(0).toUInt();
          ...
          // over all items
  QUERY_RUN("SELECT id, type FROM items", return false);
  PROGRESS_SETUP(("Update to database version 6. Migrate all GIS items."), 0, N,
                 CMainWindow::self().getBestWidgetForParent());
  progress.enableCancel(false);
  quint32 cnt = 0;
  while (query.next()) {
